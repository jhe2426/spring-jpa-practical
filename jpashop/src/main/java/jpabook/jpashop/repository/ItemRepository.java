        ...
        2. merge() 사용
            ...
            [merge() 동작 과정]
                ...
                4. 전달받은 엔티티가 가지고 있는 상태(필드 값)를 영속성 컨텍스트가 관리하는 엔티티에 복사한다.
                    영속성 컨텍스트는 같은 ID의 엔티티를 하나의 객체로만 관리한다.
                        따라서 전달받은 객체를 영속성 컨텍스트에 교체를 해서 새롭게 관리하는 것이 아니라,
                        이미 관리 중이거나 DB에서 조회한 영속 엔티티에 값만 복사한다.
                    이미 영속성 컨텍스트에 관리되는 인스턴스를 다른 여러 객체가 참조하고 있을 수 있으므로, 이 관리되는 인스턴스를 교체하지 않고
                        전달받은 인스턴스의 상태(값)만 복사하는 방식으로 merge()가 설계된 것이다.
